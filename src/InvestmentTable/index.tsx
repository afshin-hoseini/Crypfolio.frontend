...
const defaultValut : ()=>Vault =()=> ({
    remainingAmount:0, symbol: "", totalBuy:0, totalSell:0
});

const valutsDataProvider = (trades: ITradeModel[])=>{
    const vaultCollection = trades.reduce<{[k : string]: Vault}>((collection, trade)=>{
        ...
        if(trade.side === TradeSide.Buy)
            vault.totalBuy! += (trade.atPrice || 0) * (trade.amount||0);
        else 
            vault.totalSell! += (trade.atPrice || 0) * (trade.amount||0);
...
export const InvestmentTable : FC = ()=> {
    ...
    const data = React.useMemo<Vault[]>(
        () => valutsDataProvider(trades),[])
