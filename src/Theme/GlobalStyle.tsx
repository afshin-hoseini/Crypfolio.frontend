...
export const GlobalStyleInstaller: FC = () => {
  ...
  const textColorClasses = useMemo(() => {
    return Object.keys(textColors || {})
      .map(
        (key) =>
          `.typograph-color-${key}, .text-color-${key} { color: ${textColors?.[key as keyof typeof textColors]} }`
      )
      .join(' ');
  }, [textColors]);

  return useMemo(() => <GlobalStyle classes={typographClasses + textColorClasses} />, [
    textColorClasses,
    typographClasses,
  ]);
};
